...
std::vector<Pos> parseStartAndEnd(std::vector<std::string>& grid, Pos& end)
{
    std::vector<Pos> possibleStartingPoints;
    ...
    for (int i = 0; i < grid.size(); i++)
    {
        for (int j = 0; j < grid[i].size(); j++)
        {
            ...
            if (currentChar == 'S')
            {
                ...
                possibleStartingPoints.push_back(p);
            }
...
int main() {
    ...
    if (f.is_open())
    {
        ...
        std::vector<Pos> possibleStart = parseStartAndEnd(heightmap, end);

        for (int i = 0; i < possibleStart.size(); i++)
        {
            Node startNode;
            startNode.coord = possibleStart[i];
