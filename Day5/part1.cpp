    ...
    if(f.is_open())
    {
        ...
        while(getline(f, line))
        {
            ...
            else
            {
                ...
                int fromStackNb = instruction[1];
                int toStackNb = instruction[2];

                for (int i = 0; i < moveNb; i++)
                {
                    // std::vector<char>* toStack = &crateStacks[toStackNb-1];
                    int stackSize = crateStacks[fromStackNb-1].size();
                    if(stackSize <= 0)
                    {
                        break;
                    }

                    char charToMove = crateStacks[fromStackNb-1][stackSize-1];
                    
                    crateStacks[toStackNb-1].push_back(charToMove);
                    crateStacks[fromStackNb-1].pop_back();
